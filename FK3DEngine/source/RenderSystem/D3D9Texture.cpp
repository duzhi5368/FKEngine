...
namespace FK3DEngine
{
		...
		: Texture			( nWidth, nHeight, ePixelFormat, nNumLevel, Type )
	{
		...
		if( FAILED( result ) )
			Except( Exception::ERR_RENDER_API_ERROR, (String)"使用 Direct3D 9 设备创建纹理错误。"
	...
	void D3D9Texture::LockRect( void** ppBuf, UINT* pPitch, RECT* pLockRect, UINT nLevel, bool bDiscardLock )
	{
		...
		if( FAILED( result ) )
			Except( Exception::ERR_INVALIDPARAMS, (String)"锁定 Direct3D 9 纹理错误。\nD3D9 错误描述：" +
	...
	void D3D9Texture::UnlockRect( UINT nLevel )
	{
		...
		if( FAILED( result ) )
			Except( Exception::ERR_INVALIDPARAMS, (String)"解锁 Direct3D 9 纹理错误。\nD3D9 错误描述：" +
			DXGetErrorDescription9(result) );
	}
	//--------------------------------------------------------------------------------------------------------------
	//自动生成多级纹理映射
