...
{
	//--------------------------------------------------------------------------------------------------------------
	CubeTexture::CubeTexture( UINT nEdgeLength, PixelFormat ePixelFormat, UINT nNumLevel, TextureUsage Type )
		: BaseTexture		( ePixelFormat, nNumLevel, Type )
		, mEdgeLength		( nEdgeLength )
	{
	}
	//--------------------------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------------------------------------------------------------------
	//获取立方体纹理边长
	UINT CubeTexture::GetEdgeLength()
	{
		return mEdgeLength;
	}
	//--------------------------------------------------------------------------------------------------------------
	//填充指定纹理区域为某颜色
	void CubeTexture::FillColor( CubeFace eFace, DWORD color, RECT* pFillRect, UINT nLevel )
	{
		BYTE* pDstBuf = NULL;
		UINT pitch = 0;

		//计算填充宽高
		UINT width = ( pFillRect != NULL ) ? pFillRect->right - pFillRect->left : mEdgeLength;
		UINT height = ( pFillRect != NULL ) ? pFillRect->bottom - pFillRect->top : mEdgeLength;

		//锁定纹理
		LockRect( eFace, (void**)&pDstBuf, &pitch, pFillRect, nLevel );
